...
%define TD r12d      ; Top on stack, low dword
%define TB r12b      ; Top on stack, low byte
%define S r13        ; Second on stack
%define SD r13d      ; Second on stack, low dword
%define SB r13b      ; Second on stack, low dword
...
.while:
movzx ecx, byte [ebp]         ; load token at I
cmp cl, byte [jtMax]          ; break to debug if token is not in range
ja .errBadToken
movzx esi, word [ebx+2*ecx]   ; fetch jump table address offset
...
lea W, [dword datBadToken1]   ; print bad token error label
call mStrPut.W
movzx W, byte [ebp]           ; print offending token value
call mDotB.W
lea W, [dword datBadToken2]   ; print token's instruction pointer
call mStrPut.W
mov W, rbp
sub W, initROM
call mDotB.W
...
movzx eax, word [ebp]  ; get length of string in bytes (for adjusting I)
add ax, 2              ;   add 2 for length dword
mov WD, ebp            ; I (ebp) should be pointing to {length, chars}
...
mov edx, DStackLo
mov [edx+8*DSHead], S  ; on entry, DSHead points to an availble cell
inc DSHead
...
shl WD, 24             ; Prepare W as string struct {db 1, 0, ascii, 0}
shr WD, 8
mov WB, 1
...
shl W, 56             ; shift low byte to high byte since mDot starts there
mov ecx, 2            ; set digit count
jmp mDot.W_ecx        ; use the digit conversion loop from mDot

mDot:                 ; Print T to stdout (16 hex digits)
mov W, T
call mDrop
.W:                   ; Print W to stdout (16 hex digits)
mov ecx, 16
...
.for:
mov r8, W             ; get the high nibble of W
shr r8, 60
...
mov word [edi], 4              ;   print label for T
mov dword [edi+2], 0x2054200A  ;   CR, " T "
mov WD, dword Pad
call mStrPut.W
...
je .done
lea edi, [Pad]
mov word [edi], 4              ;   print label for S
mov dword [edi+2], 0x2053200A  ;   CR, " S "
mov WD, dword Pad
call mStrPut.W
...
mov ebp, DSHead
.for:
mov WD, DSLen         ; print the stack depth label
sub WD, ebx
inc WD
call mDotB.W
call mSpace
...
mov WD, [esi+8*ebp]   ; peek at the current cell's value
