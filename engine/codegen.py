...
C_HEADER_OUTFILE = "mkb_autogen.h"
C_CODE_OUTFILE = "mkb_autogen.c"
...
def c_opcode_constants():
  ...
  for (i, line) in enumerate(filter(OPCODES)):
    (name, opcode) = line.strip().split(" ")
    ops += [f"#define MK_{opcode.upper():6} ({i:2})"]
  return "\n".join(ops)
...
def c_core_vocab():
  ...
  for line in filter(MEMORY_MAP):
    (addr, name) = line.split(" ")
    key = f"{{\"{name}\"}},"
    cv += [f"""{tab()}{{ {key:16} MK_T_CONST, 0x{addr:7} }},"""]
  for line in filter(CONSTANTS):
    (name, code) = line.split(" ")
    key = f"{{\"{name}\"}},"
    cv += [f"""{tab()}{{ {key:16} MK_T_CONST, {code:9} }},"""]
  for (i, line) in enumerate(filter(OPCODES)):
    (name, code) = line.strip().split(" ")
    if name == '<ASM>':
      continue
    key = f"{{\"{name}\"}},"
    cv += [f"""{tab()}{{ {key:16} MK_T_OP,    MK_{code:6} }},"""]
  return "\n".join(cv)
...
{tab()}const char * const name[MK_VOC_ITEM_NAME_LEN];
{tab()}const uint8_t type_code;
{tab()}const uint32_t value;
}} mk_voc_item_t;
